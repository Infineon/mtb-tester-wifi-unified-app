...
#ifdef ULP_SUPPORT
    if(is_sta_network_up == true)
    {
       extern whd_interface_t whd_ifs[WHD_INTERFACE_MAX];
       uint32_t set_ulp_mode = ULP_DS2_SUPPORT;
       whd_wifi_config_ulp_mode(whd_ifs[CY_WCM_INTERFACE_TYPE_STA], &set_ulp_mode, &inactive_window_ms);
     }
#endif
...
#ifdef ULP_SUPPORT
    extern cy_rslt_t suspend_console_and_debug_tasks(void);
    extern cy_rslt_t resume_console_and_debug_tasks(void);
#endif
...
#ifdef ULP_SUPPORT
        suspend_console_and_debug_tasks();
#endif
...
#ifdef ULP_SUPPORT
            resume_console_and_debug_tasks();
#endif
