...
extern cy_rslt_t wl_uart_getchar(char* c);
extern cy_rslt_t wl_uart_putchar(char c);
 ...
 *
 ******************************************************************************/
int wl_remote_command_handler(  void )
{
    int result = 0;
...
int wl_remote_tx_response( void* buf_ptr, int cmd)
{
    ...
    if ( rem_cdc.msg.flags & REMOTE_GET_IOCTL )
    {
       ...
       cy_wcm_get_whd_interface(CY_WCM_INTERFACE_TYPE_STA, &sta_interface);
       whd_ensure_wlan_bus_not_in_deep_sleep(sta_interface->whd_driver);
#endif
